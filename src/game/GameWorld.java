...
public class GameWorld extends JPanel {
    ...
    public void startSprites() {
    	...
    	for(TrainerSprite sprite: this.sprites) {
    		if (threads.size()<sprites.size()){
	...
	public void enterBattleArea (TrainerSprite sprite) throws InterruptedException {
		...
		try {
			...
			//if no trainer waiting in the battle field
			if(waitingSprite==null) {
				waitingSprite=sprite;
				trainer1=sprite.getTrainer();
				window.setTrainer1(trainer1);
				battleCondition.await();
				
			}else{
				//battle the waiting trainer
				trainer2=sprite.getTrainer();
				window.setTrainer2(trainer2);
				trainer1=waitingSprite.getTrainer();
				
				Trainer winner=Trainer.doBattle(trainer1, trainer2);
				Thread.sleep(1000);
				//set the winner trainer
				if (winner.equals(trainer2)) {
